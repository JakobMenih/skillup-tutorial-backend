...
export abstract class AbstractService {
  ...
  async findAll(relations = []): Promise<any[]> {
    try {
      ...
      Logging.error(error)
      throw new InternalServerErrorException('Something went wrong while searching for list of elements.')
    }
  }
  ...
  async paginate(page = 1, relations = []): Promise<PaginatedResult> {
    ...
    try {
      ...
      Logging.error(error)
      throw new InternalServerErrorException('Something went wrong while searching for a paginated with an element.')
    }
  }
}
